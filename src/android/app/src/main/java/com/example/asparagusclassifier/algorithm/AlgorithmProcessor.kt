...
object AlgorithmProcessor {
    ...
    fun processImage(bitmap: Bitmap, calibration: CalibrationData?, viewMode: Int = 2): AlgorithmResult {
        ...
        return useMatScope { scope ->
            ...
            try {
                ...
                val poseInfo = if (AlgorithmConfig.isAruco3DEnabled && allImagePoints.total().toInt() >= 4 && calibration != null && calibration.isValid()) {
                ...
                return@useMatScope AlgorithmResult(
                    ...
                    axis3DPoints = if (AlgorithmConfig.isWorldCoordEnabled) PoseEstimator.projectAxes(poseInfo) else null,
    ...
    fun processRealtimePose(bitmap: Bitmap, calibration: CalibrationData?): AlgorithmResult {
        return useMatScope { scope ->
            ...
            try {
                ...
                val poseInfo = if (AlgorithmConfig.isAruco3DEnabled && allPoints.total().toInt() >= 4 && calibration != null && calibration.isValid()) {
                ...
                return@useMatScope AlgorithmResult(
                    ...
                    cameraPosWorld = poseInfo?.cameraPosWorld,
                    axis3DPoints = if (AlgorithmConfig.isWorldCoordEnabled && poseInfo != null) PoseEstimator.projectAxes(poseInfo) else null,
                    markerAxes = arucoResult.markerMap.mapValues { (id, corners) ->
    ...
    private fun generateDiagnosticStrip(sourceMat: Mat, axis: List<PointF>, baseline: List<PointF>?): Bitmap? {
        ...
        val m = DoubleArray(6); rotMat.get(0, 0, m)
